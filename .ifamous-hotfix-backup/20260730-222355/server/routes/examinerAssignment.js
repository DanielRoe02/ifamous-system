...
router.use(authenticateToken, requireAnyRole("coordinator"));
    ...
    const result = await transaction(async (connection) => {
      ...
      await connection.query(
        `UPDATE fyp_projects
         SET examiner_user_id = ?, examiner_name = ?, examiner_email = ?,
             status = 'Examiner Assigned', current_phase = 'Examination', updated_at = NOW()
         WHERE project_id = ?`,
      ...
      await connection.query(
        `INSERT INTO fyp_notifications
         ...
         VALUES (?, 'Examiner', ?, ?, 'FYP Examination Assigned', ?, 0, NOW())`,
        [project.project_id, eligible[0].full_name, eligible[0].email, `You have been assigned to examine "${project.project_title}".`]
      );
      return { assignmentId: assignment.insertId, examiner: eligible[0] };
    });
    ...
    await transaction(async (connection) => {
      ...
      await connection.query(
        `UPDATE fyp_projects SET examiner_user_id = NULL, examiner_name = NULL, examiner_email = NULL,
         status = 'Awaiting Examiner Assignment', current_phase = 'Examiner Assignment', updated_at = NOW()
         WHERE project_id = ?`,
        [req.params.projectId]
      );
    });
  ...
  try {
    ...
    await query(
      `UPDATE fyp_projects SET status = 'Result Released', result_status = 'Released',
       final_score = ?, result_released_at = NOW(), current_phase = 'Completed', updated_at = NOW()
       WHERE project_id = ?`,
    ...
    res.json({ success: true, status: "Result Released", finalScore });
  } catch (error) {
    return sendError(res, error);
  }
});
